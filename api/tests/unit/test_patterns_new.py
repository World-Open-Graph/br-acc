...
from bracc.services.pattern_service import PATTERN_QUERIES, run_pattern
...
@pytest.mark.anyio
async def test_run_pattern_passes_all_threshold_params() -> None:
    """Verify pattern service passes all config-based threshold params to Neo4j."""
    mock_record = MagicMock()
    mock_record.__iter__ = lambda self: iter(["pattern_id"])
    mock_record.__getitem__ = lambda self, key: {"pattern_id": "debtor_contracts"}[key]
    ...
    with patch("bracc.services.pattern_service.execute_query", new_callable=AsyncMock) as mock_eq:
        mock_eq.return_value = [mock_record]
        await run_pattern(session, "debtor_contracts", entity_id="test-id")

    mock_eq.assert_awaited_once()
    _session, _query_name, params = mock_eq.await_args.args[:3]

    assert params["temporal_window_years"] == settings.pattern_temporal_window_years
    assert params["min_contract_value"] == settings.pattern_min_contract_value
    assert params["min_contract_count"] == settings.pattern_min_contract_count
    assert params["min_debt_value"] == settings.pattern_min_debt_value
    assert params["same_as_min_confidence"] == settings.pattern_same_as_min_confidence
    assert params["pep_min_confidence"] == settings.pattern_pep_min_confidence
    assert params["min_recurrence"] == settings.pattern_min_recurrence
    assert params["min_discrepancy_ratio"] == settings.pattern_min_discrepancy_ratio
    assert params["entity_id"] == "test-id"
    assert params["include_probable"] is False
    assert set(params.keys()) == {
        ...
        "min_contract_count",
        "min_debt_value",
        "same_as_min_confidence",
        "pep_min_confidence",
        "min_recurrence",
        "min_discrepancy_ratio",
    }
...
async def test_debtor_contracts_returns_results() -> None:
    ...
    assert results[0].pattern_name == "Coocorrência: dívida ativa e contratos"
...
async def test_embargoed_receiving_returns_results() -> None:
    ...
    assert results[0].pattern_name == "Coocorrência: embargo e recursos públicos"
...
async def test_amendment_beneficiary_contracts_returns_results() -> None:
    ...
    assert results[0].pattern_name == "Coocorrência: emenda e contratos"
